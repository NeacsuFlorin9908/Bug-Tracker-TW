...
function validLogin(Model, payload) {
	return Object.entries(Model.tableAttributes).reduce((valid, [email, field]) => {
		if (
			valid && 
			!field._autoGenerated && 
			!field.primaryKey && 
...
async function postRecordLogin(Model, request, response) {
	
	let emailExists = await Model.findOne({ where: { email: request.body.email } }); //find by primery key
	let passwordExists = await Model.findOne({ where: { user_password: request.body.user_password } }); //find by primery key
